...
def sample_embeddings(embedding_dim):
    ...
    user_embeddings = {
        f"user_{i}": np.random.normal(0, 0.1, size=embedding_dim) for i in range(10)
    }

    # Create sample item embeddings
    item_embeddings = {
        f"item_{i}": np.random.normal(0, 0.1, size=embedding_dim) for i in range(20)
    }
...
class TestRecommenderSystem:
    ...
    def test_recommend(self, multi_modal_model, embedding_manager):
        ...
        recommendations = recommender.recommend(
            user_id="user_0", k=5, context={"location": "home"}
        )
    ...
    def test_train(self, multi_modal_model):
        ...
        try:
            ...
            pytest.skip(
                "Train method not implemented or requires different data format"
            )
