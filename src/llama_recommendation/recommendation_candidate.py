...
class CandidateGenerator:
    ...
    def generate(
        ...
        if not available_items:
            self.logger.warning(
                f"No available items for user {user_id} after exclusions"
            )
            return []
        ...
        for strategy in self.strategies:
            try:
                ...
                for candidate in strategy_candidates:
                    candidate["strategies"] = candidate.get("strategies", []) + [
                        strategy
                    ]
                    ...
                    existing = next(
                        (c for c in candidates if c["item_id"] == candidate["item_id"]),
                        None,
                    )
    ...
    def _similarity_strategy(
        ...
        for item_id in available_items:
            ...
            candidates.append(
                {"item_id": item_id, "score": similarity, "similarity": similarity}
            )
    ...
    def _popularity_strategy(self, available_items: List[str]) -> List[Dict[str, Any]]:
        ...
        for item_id in available_items:
            ...
            candidates.append(
                {"item_id": item_id, "score": popularity, "popularity": popularity}
            )
