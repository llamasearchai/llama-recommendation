...
class FederatedClient:
    ...
    def update_local_model(
        ...
        try:
            ...
            # Create DP trainer
            trainer = DPTrainer(
                model=self.model, epsilon=update_epsilon, delta=self.dp_delta
            )
    ...
    def _extract_model_parameters(self) -> Dict[str, np.ndarray]:
        ...
        # Fallback to model embeddings as parameters
        elif hasattr(self.model, "user_embeddings") and hasattr(
            ...
            self.logger.warning(
                "Could not extract model parameters, returning empty dict"
            )
            return {}
    ...
    def _clip_update(
        ...
        if total_norm > clip_norm:
            ...
            self.logger.info(
                f"Clipped update norm from {total_norm:.4f} to {clip_norm:.4f}"
            )
    ...
    def send_model_update(
        ...
        try:
            ...
            if response.status_code == 200:
                ...
                error_msg = (
                    f"Error sending update: {response.status_code} - {response.text}"
                )
    ...
    def pull_global_model(self) -> Dict[str, Any]:
        ...
        try:
            ...
            if response.status_code == 200:
                ...
                error_msg = (
                    f"Error pulling model: {response.status_code} - {response.text}"
                )
                self.logger.error(error_msg)
                return {"error": error_msg}
    ...
    def check_server_status(self) -> Dict[str, Any]:
        ...
        try:
            ...
            if response.status_code == 200:
                ...
                error_msg = f"Error checking server status: {response.status_code} - {response.text}"
                self.logger.error(error_msg)
                return {"error": error_msg}
    ...
    def get_privacy_report(self) -> Dict[str, Any]:
        ...
        report = {
            "client_id": self.client_id,
            "initial_budget": self.dp_epsilon
            * 10,  # Assuming initial budget was 10 times epsilon
            "remaining_budget": self.privacy_budget,
