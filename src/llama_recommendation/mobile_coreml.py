...
class CoreMLConverter:
    ...
    def _convert_scoring_model(self, ct) -> "MLModel":
        ...
        def scoring_function(user_embedding, item_embedding):
            ...
            concatenated = np.concatenate(
                [user_embedding_np, item_embedding_np], axis=1
            )
            ...
            hidden1 = np.maximum(
                0,
                np.dot(
                    concatenated, np.random.randn(2 * self.model.embedding_dim, 256)
                ),
            )
    ...
    def _convert_embedding_model(self, ct) -> "MLModel":
        ...
        user_model = ct.convert(
            user_embedding_function,
            convert_to="mlprogram",
            compute_units=ct.ComputeUnit.CPU_AND_NE,
        )

        item_model = ct.convert(
            item_embedding_function,
            convert_to="mlprogram",
            compute_units=ct.ComputeUnit.CPU_AND_NE,
        )
        ...
        pipeline = ct.models.pipeline.Pipeline(
            ...
            output_features=[
                ct.TensorType(name="embedding", shape=(1, self.model.embedding_dim))
            ],
        )
    ...
    def _convert_complete_model(self, ct) -> "MLModel":
        ...
        inputs = [
            ct.TensorType(name="user_id", shape=(1,), dtype=np.int32),
            ct.TensorType(
                name="candidate_item_ids", shape=(ct.RangeDim(1, 100),), dtype=np.int32
            ),
            ct.TensorType(name="context_features", shape=(1, 10), optional=True),
        ]
        ...
        outputs = [
            ...
            ct.TensorType(
                name="ranked_item_ids", shape=(ct.RangeDim(1, 100),), dtype=np.int32
            ),
        ]
    ...
    def _load_user_embeddings(self) -> None:
        ...
        try:
            self.user_embeddings = np.load(
                self.user_embeddings_path, allow_pickle=True
            ).item()
            self.logger.info(
                f"Loaded user embeddings for {len(self.user_embeddings)} users"
            )
        except Exception as e:
    ...
    def _load_item_embeddings(self) -> None:
        ...
        try:
            self.item_embeddings = np.load(
                self.item_embeddings_path, allow_pickle=True
            ).item()
            self.logger.info(
                f"Loaded item embeddings for {len(self.item_embeddings)} items"
            )
        except Exception as e:
