...
class Ranker:
    ...
    def rank(
        ...
        scored_items = self._score_candidates(
            user_id, candidates, context, causal_estimation
        )
        ...
        if self.position_bias_correction:
            ...
            # Sort by final score
            ranked_items = sorted(
                diversified_items, key=lambda x: x["score"], reverse=True
            )
            ...
            if best_item_idx >= 0:
                ...
                if len(selected) > 0:
                    ...
                    for sel in selected:
                        sel_id = sel["item_id"]
                        similarity = self._cosine_similarity(
                            item_embeddings[best_item["item_id"]],
                            item_embeddings[sel_id],
                        )
    ...
    def _correct_position_bias(
        ...
        for i, item in enumerate(sorted_items):
            ...
            confidence = 1.0 - (
                i / len(sorted_items) * 0.5
            )  # Higher ranks have higher confidence
