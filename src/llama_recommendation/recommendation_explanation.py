...
class ExplanationGenerator:
        ...
        for exp_type in self.explanation_types:
            try:
                if exp_type == "similarity":
                    component = self._generate_similarity_explanation(
                        user_id, item_id, context
                    )
                elif exp_type == "popularity":
                    component = self._generate_popularity_explanation(
                        item_id, item_metadata
                    )
                elif exp_type == "category":
                    component = self._generate_category_explanation(
                        user_id, item_id, context
                    )
                elif exp_type == "collaborative":
                    component = self._generate_collaborative_explanation(
                        user_id, item_id, context
                    )
                elif exp_type == "diversity":
                    component = self._generate_diversity_explanation(
                        user_id, item_id, context
                    )
                elif exp_type == "fairness":
                    component = self._generate_fairness_explanation(
                        user_id, item_id, context
                    )
                elif exp_type == "trending":
                    component = self._generate_trending_explanation(
                        item_id, item_metadata
                    )
                elif exp_type == "causal":
                    component = self._generate_causal_explanation(
                        user_id, item_id, context
                    )
                else:
    ...
    def _generate_category_explanation(
        ...
        matching_categories = [
            cat for cat in item_categories if cat in preferred_categories
        ]
    ...
    def get_counterfactual_explanation(
        ...
        try:
            ...
            return (
                "If certain features were different, the recommendation might change."
            )
