...
class CounterfactualExplainer:
    ...
    def generate_explanation(
        ...
        counterfactual_evaluations.sort(
            key=lambda x: target_direction * x["change"], reverse=True
        )
    ...
    def _identify_important_features(
        ...
        # Compute gradients
        user_grad, item_grad = mx.grad(predict, argnums=(0, 1))(
            user_emb_mx, item_emb_mx
        )
    ...
    def _generate_textual_explanation(
        ...
        if len(feature_changes) == 1:
            ...
            features_text = (
                ", ".join(feature_changes[:-1]) + f", and {feature_changes[-1]}"
            )
...
class CausalSimulator:
    ...
    def __init__(
        self, base_model: "BaseModel", logger: Optional[logging.Logger] = None
    ):
        """
    ...
    def simulate_intervention(
        ...
        intervention_scores = self.base_model.predict(
            user_id, item_ids, modified_context
        )
    ...
    def _apply_intervention(
        ...
        if intervention_type == "context_modification":
            ...
            modified_context["feature_modifications"] = intervention.get(
                "modifications", []
            )

        elif intervention_type == "embedding_modification":
            # Add embedding modifications to context
            modified_context["embedding_modifications"] = intervention.get(
                "modifications", []
            )
    ...
    def evaluate_treatment_policy(
        ...
        for user_id, item_id in samples:
            ...
            if treated:
                ...
                with_treatment = float(
                    self.base_model.predict(user_id, [item_id], context)[0]
                )
        ...
        if untreated_samples:
            ...
            untreated_outcomes = np.mean(
                [o["without_treatment"] for o in untreated_samples]
            )
