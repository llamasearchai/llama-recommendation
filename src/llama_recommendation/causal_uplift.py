...
class UpliftModel:
    ...
    def _create_model(self, hidden_dims: List[int]) -> nn.Module:
        ...
        class UpliftEstimator(nn.Module):
            ...
            def __call__(
                self, user_embedding: mx.array, item_embedding: mx.array
            ) -> mx.array:
        ...
        return UpliftEstimator(
            embedding_dim=self.embedding_dim, hidden_dims=hidden_dims
        )
    ...
    def train(
        ...
        for epoch in range(epochs):
            ...
            train_loss = []
            for i in tqdm(
                range(0, len(train_df), batch_size), desc=f"Epoch {epoch + 1}/{epochs}"
            ):
